...
bool MoveAvalibleExceptCorner12(char board[26][26], int n, char colour){
    bool available = false;
    for(int i = 0; i<n; i++){
        for(int j = 0; j<n; j++){
            if(isValidMove(board, n, colour, i, j) && !((i == 0 && (j == 1 || j == n-2)) || (i == n-1 && (j == 1 || j == n-2)) 
                ||  (j == 0 && (i == 1 || i == n-2))|| (j == n - 1 && (i == n-2 || i == 1)) 
                || (i == 1 && (j == 1 || j == n-2)) || (i == n-2 && (j ==1 || j == n-2)))){
                available = true;
            }
        }
    }
...
bool checkBlankinFour(char board[26][26], int row, int col){
    bool occupied = false;
    if(board[row+1][col+1] != 'U' && board[row-1][col-1] != 'U' && board[row-1][col] != 'U' && board[row+1][col] != 'U'){
        occupied = true;    
    }
    return occupied;
}

bool quietMove(char board[26][26], int n, char colour){
    bool quiet = false;
    for(int i = 0; i<n; i++){
        for(int j = 0; j<n; j++){
            if(isValidMove(board, n, colour, i, j) && checkBlankinFour(board, i, j) && (i >= 1 && j >= 1 && i <= n-2  && j <= n-2)){
                quiet = true;
            }
        }
    }
        ...
        for(int j = 0; j < n; j++){
            if(isValidMove(board, n, turn, i, j)){
                if((i == 0 && j == 0) || (i == n-1 && j == 0) || (i == 0 && j == n-1) || (i == n-1 && j == n-1)){
                    ...
                    *col = j;
                    return 0;
                }else if((i == 0 && j < n-2 && j > 1) || (j == 0 && i > 1 && i < n-2) || (i == n-1 && j < n-2 && j > 1) || (j == n-1 && i > 1 && i < n-2 )){
                    *row = i;
                    *col = j;
                    return 0;
                }else if(((i == 0 && (j == 1 || j == n-2)) || (i == n-1 && (j == 1 || j == n-2)) 
                ||  (j == 0 && (i == 1 || i == n-2))|| (j == n - 1 && (i == n-2 || i == 1)) 
                || (i == 1 && (j == 1 || j == n-2)) || (i == n-2 && (j ==1 || j == n-2))) && MoveAvalibleExceptCorner12(board, n, turn)){
                    continue;
                }else if(quietMove(board, n, turn)){
                    *row = i;
                    *col = j;
                    return 0;
                }else{
